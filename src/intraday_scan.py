...
def load_rvol_reference(
    ...
    print(
        f"Supabase RVOL reference: {len(rows)} rows; "
...
def fetch_price_board(symbols: list[str], deadline: float) -> pd.DataFrame:
    ...
    - Het ngan sach thoi gian thi dung fallback/retry, tra PARTIAL neu van >=80%.
    """
    ...
    for offset in range(0, len(requested), PRICE_BOARD_BATCH_SIZE):
        ...
        # Chi VCI cuu dung cac ma KBS con thieu, khong quet lai ca batch.
...
def filter_newer_stock_snapshot_rows(
    ...
    """Khong cho mot run cu ghi de stock_snapshot da moi hon."""
    if not rows:
        return rows
    response = supabase_request(
        "GET",
...
def build_rvol_reference(
    ...
    if ref.empty:
        return (
            ...
            pd.DataFrame(columns=["symbol", "avg_volume_30m_10", "rvol30_sessions"]),
        )
    ...
    if missing:
        ...
        return pd.DataFrame(columns=["symbol", "start_volume_today"]), pd.DataFrame()
    ...
    # Supabase time co the tra HH:MM:SS; chuan hoa ve HH:MM de khop slot Python.
    ...
    historical = historical[historical["trading_date"].isin(historical_dates)]
...
def calculate_rvol(
    ...
    output["volume_30m"] = (
        output["volume_accumulated"] - output["start_volume_today"]
    )
    ...
    if (
        ...
        stats = (
            pivot.groupby("symbol")["historical_volume_30m"]
            .agg(["mean", "count"])
            .reset_index()
            .rename(
                columns={
                    "mean": "avg_volume_30m_10",
                    "count": "rvol30_sessions",
                }
            )
        )
        output = (
            output.drop(columns=["avg_volume_30m_10", "rvol30_sessions"])
            .merge(stats, on="symbol", how="left")
        )
        output["rvol30_sessions"] = (
            output["rvol30_sessions"].fillna(0).astype(int)
        )
...
def main() -> None:
    ...
    if in_market:
        ...
        current_slot = scan_at.strftime("%H:%M")
        start_slot = None
    ...
    result["exchange"] = result.get(
        "exchange_market", result.get("exchange_watchlist", "")
    )
    result = result.drop(
    ...
    if in_market:
        reference = load_rvol_reference(scan_at, current_slot, start_slot)
    ...
    if in_market:
        result["signal_price_3pct"] = (
            result["price_change_pct"] >= PRICE_THRESHOLD_PCT
        )
    ...
    if forced_outside_session:
        ...
        snapshots = dashboard[SNAPSHOT_COLUMNS].copy()
    ...
    run_message = (
        ...
        if forced_outside_session
            ...
            f"slot={current_slot}; rvol_start={start_slot}; "
    ...
    # Supabase la primary. Cac upsert deu idempotent theo primary key.
    if snapshot_rows:
        upsert_rows(
            "intraday_snapshots",
            snapshot_rows,
    ...
    # Sheet/GAS chi la backup tam thoi cho website cu.
    ...
    print(
        f"DONE: {successful}/{len(watchlist)} ma; "
