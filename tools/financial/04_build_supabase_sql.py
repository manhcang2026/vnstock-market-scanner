...
# Supabase SQL Editor không nhận query quá lớn.
# 250 rows/file đủ nhỏ nhưng vẫn không tạo quá nhiều file.
QUARTERLY_ROWS_PER_FILE=250
LATEST_ROWS_PER_FILE=250
...
def insert_sql(table,cols,rows,conflict):
    if not rows:
        return f"-- Không có dữ liệu cho {table}\n"
    parts=["begin;","",f"-- UPSERT {len(rows)} rows vào {table}. KHÔNG DELETE/TRUNCATE."]
    allcols=cols+["updated_at"]
    parts.append(f"insert into public.{table} (\n  "+", ".join(allcols)+"\n) values")
    vals=[]
    for r in rows:
        vals.append("  ("+", ".join([sv(c,r.get(c)) for c in cols]+["now()"])+")")
    parts.append(",\n".join(vals))
...
def cleanup_old_parts(out: Path):
    # Xóa file split cũ trước khi tạo lại để không lẫn part 1000-row cũ.
    patterns=[
        "02_financial_quarterly_part*.sql",
        "03_financial_latest*.sql",
    ]
    removed=[]
    ...
    if removed:
        print(f"Removed old SQL parts: {len(removed)}")

def write_parts(out: Path,prefix: str,table: str,cols:list[str],rows:list[dict],conflict:list[str],rows_per_file:int):
    created=[]
    for i,st in enumerate(range(0,len(rows),rows_per_file),1):
        batch=rows[st:st+rows_per_file]
        path=out/f"{prefix}_part{i:02d}.sql"
        path.write_text(insert_sql(table,cols,batch,conflict),encoding="utf-8")
        created.append((path,len(batch)))
    return created
...
def main():
    ...
    if bad:
        raise RuntimeError("Còn mã chưa duyệt: "+", ".join(bad[:30]))

    metadata=[]
    for ir in industry:
        s=(ir.get("symbol") or "").upper()
        wr=watch.get(s,{})
        metadata.append({
            "symbol":s,
            "exchange":wr.get("exchange",ir.get("exchange","")),
            "company_name":wr.get("organ_name",ir.get("company_name","")),
            "display_name":ir.get("display_name") or wr.get("organ_name",""),
    ...
    cleanup_old_parts(out)

    # Metadata nhỏ, giữ 1 file.
    (out/"01_stock_metadata.sql").write_text(
        insert_sql("stock_metadata",META_COLS,metadata,["symbol"]),
        encoding="utf-8"
    )

    qparts=write_parts(
        out,
        "02_financial_quarterly",
        "financial_quarterly",
        FIN_Q_COLS,
        quarterly,
        ["symbol","period"],
        QUARTERLY_ROWS_PER_FILE,
    )

    # Latest cũng split chủ động để tránh gặp lại lỗi "Query is too large".
    lparts=write_parts(
        out,
        "03_financial_latest",
        "financial_latest",
...
"""
    ...
    print("=== SQL EDITOR SAFE SPLIT ===")
    print(f"Metadata: {len(metadata)} rows -> 1 file")
    print(f"Quarterly: {len(quarterly)} rows -> {len(qparts)} files, max {QUARTERLY_ROWS_PER_FILE} rows/file")
    print(f"Latest: {len(latest)} rows -> {len(lparts)} files, max {LATEST_ROWS_PER_FILE} rows/file")
    print("Quarterly files:")
    for p,n in qparts:
        print(f"  {p.name}: {n} rows")
    print("Latest files:")
    for p,n in lparts:
        print(f"  {p.name}: {n} rows")
    print("XONG. Chưa tự ghi Supabase.")
