(function () {
  ...
  function signalRailHtml(row) {
    ...
    return '<div class="signal-rail" aria-label="' + count + ' trên 4 tín hiệu"><div class="signal-segments">' + flags.map(function (value, index) { return '<span class="signal-segment ' + (value ? 'on' : '') + '" title="' + esc(labels[index]) + '"></span>'; }).join("") + '</div><strong>' + count + '/4</strong>' + (status ? '<em>' + status + '</em>' : '') + '</div>';
  }
  ...
  function overviewRowHtml(row, sample) {
    ...
    return '<article class="overview-stock-row' + (sample ? ' is-sample' : '') + '">' +
      ...
      '<div class="overview-stock-evidence"><span>RVOL30 <b>' + (rvol === null ? '—' : formatNumber(rvol, 0) + '%') + '</b></span><span>MA200 <b class="' + tone(row.ma200_distance_pct) + '">' + pct(row.ma200_distance_pct, 1) + '</b></span></div>' +
  ...
  function mountShell() {
    app.innerHTML = '' +
      '<header class="app-header">' +
        ...
        '</div>' +
      '</header>' +
      '<aside class="desktop-sidebar" aria-label="Thanh điều hướng"><nav id="desktop-nav-links" aria-label="Điều hướng chính"></nav><div class="sidebar-footer"><div id="desktop-account-link"></div><span class="staging-badge"><strong>STAGING</strong><small>v19.1.2-parity</small></span></div></aside>' +
