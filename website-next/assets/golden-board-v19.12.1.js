(function () {
  ...
  function guestRightRailHtml(data, teaserCount) {
    ...
    return '<aside class="gb-right-rail" aria-label="Thông tin Bảng vàng">' +
      '<section class="gb-rail-card">' +
        ...
        '<p>Một mã được ghi danh khi CCC đã thực sự ghi nhận đủ <b>4/4</b> trong phiên. Mã vẫn nằm lại trong lịch sử kể cả khi tín hiệu sau đó giảm.</p>' +
      ...
      '</section>' +
      '<section class="gb-rail-card">' +
        '<span class="eyebrow">QUYỀN ĐANG XEM</span>' +
        '<h3>Khách · Hé một chút</h3>' +
        '<p>Hiển thị tối đa <b>2 mã thật</b> sau ' + fmt(delay, 0) + ' phút. Chi tiết kỹ thuật đầy đủ vẫn theo quyền thành viên.</p>' +
      ...
      '<section class="gb-rail-card gb-rail-note">' +
        ...
        '<p>Bảng vàng là nhật ký tín hiệu của CCC, không phải khuyến nghị mua bán. Thời điểm hiển thị là lúc hệ thống ghi nhận, không khẳng định đó là điểm mua.</p>' +
      '</section>' +
    '</aside>';
  }
  ...
  function memberRightRailHtml(data, rows) {
    ...
    return '<aside class="gb-right-rail" aria-label="Tóm tắt Bảng vàng">' +
      ...
      '</section>' +
      weekCard +
      '<section class="gb-rail-card gb-rail-note"><span class="eyebrow">CÁCH ĐỌC</span><p><b>Đang đạt 4/4</b> là trạng thái hiện tại. <b>Bảng vàng</b> lưu mọi mã đã từng được CCC ghi nhận 4/4 trong phiên.</p></section>' +
    '</aside>';
  }
  ...
  function fullPageHtml(payload) {
    ...
    if (!payload.signedIn) {
      ...
      return '<div class="page-shell golden-board-page">' +
        '<header class="page-header gb-page-header"><div class="page-heading"><div><span class="eyebrow">NHẬT KÝ TÍN HIỆU CCC</span><h1>🏆 Bảng vàng</h1><p>Sổ ghi danh những mã đã được CCC ghi nhận đạt đủ 4/4 trong phiên.</p></div><div class="gb-date-static">Phiên ' + esc(dateText(data.trading_date)) + "</div></div></header>" +
        '<div class="gb-content-grid">' +
          '<div class="gb-content-main">' +
            '<section class="gb-hero-grid gb-hero-grid-guest"><div class="gb-stat"><span>Đã ghi danh</span><strong>' + fmt(marketTotal, 0) + '</strong><small>mã trong phiên</small></div><div class="gb-stat"><span>Đang xem</span><strong>Khách</strong><small>tối đa 2 mã · trễ ' + fmt(data.teaser_delay_minutes || 20, 0) + " phút</small></div></section>" +
    ...
    return '<div class="page-shell golden-board-page">' +
      '<header class="page-header gb-page-header"><div class="page-heading"><div><span class="eyebrow">NHẬT KÝ TÍN HIỆU CCC</span><h1>🏆 Bảng vàng</h1><p>Mã đã từng được hệ thống ghi nhận đạt đủ 4/4 sẽ được giữ lại để xem lịch sử, kể cả khi sau đó tín hiệu giảm.</p></div>' + datesHtml(data) + "</div></header>" +
      '<div class="gb-content-grid">' +
        '<div class="gb-content-main">' +
