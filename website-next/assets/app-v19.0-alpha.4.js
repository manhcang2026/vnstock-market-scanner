(function () {
  ...
  var state = {
    ...
    query: "", exchange: "all", signal: new URLSearchParams(location.search).get("signal") || "",
  ...
  function signalRailHtml(row) {
    var labels = ["Giá tăng từ 3%", "KL ngày từ 200% KLTB10", "Trên MA200", "RVOL30 từ 200%"];
    var values = [row.signalPrice3pct, row.signalVolume200pct, row.signalAboveMa200, row.signalRvol30_200pct];
    var dots = values.map(function (passed, index) { return '<span class="rail-dot '+(passed?'is-on':'is-off')+'" aria-hidden="true" title="'+esc(labels[index])+'">'+(passed?'●':'○')+'</span>'; }).join('');
    return '<span class="signal-rail" aria-label="'+row.signalCount+' trên 4 tín hiệu đang đạt">'+dots+'<b>'+row.signalCount+'/4</b></span>';
  }
  ...
  function overviewHtml() {
    ...
    var count4 = discoveryRows("4of4").length;
    var count3plus = discoveryRows("3plus").length;
    var count2plus = discoveryRows("2plus").length;
    var countEarly = discoveryRows("rvol30").length;
    var watchlist = mockWatchlistRows();
    var plan = mockPlan();
    var watchlistStrong = watchlist.filter(function(row){return row.signalCount >= 3;}).length;
    var remaining = plan.changeLimit === null ? null : Math.max(0, plan.changeLimit - state.mockChangeUsed);
    var marketMetrics = [
      ["4of4","Đạt 4/4",count4,"Đủ bốn điều kiện scanner"],
      ["3plus","Từ 3 tín hiệu",count3plus,"Bao gồm nhóm 4/4"],
      ["2plus","Từ 2 tín hiệu",count2plus,"Bao gồm nhóm từ 3 tín hiệu"],
      ["rvol30","RVOL30 sớm",countEarly,"RVOL30 từ 200%"]
    ].map(function(metric){return '<button type="button" class="market-metric discovery-trigger" data-discovery="'+metric[0]+'"><span>'+esc(metric[1])+'</span><strong>'+metric[2]+'</strong><small>'+esc(metric[3])+'</small><b>Xem nhóm</b></button>';}).join('');
    var discoveryKeys = ["4of4","3plus","2plus","rvol30"];
    var discoveryHtml = discoveryKeys.map(function(key){
      var data=discoveryData(key);
      return '<button type="button" class="discovery-card discovery-trigger" data-discovery="'+key+'"><div><strong>'+data.totalCount+' '+esc(discoveryLabel(key))+'</strong><span>Bạn xem được '+data.visibleCount+'/'+data.totalCount+'</span></div>'+(data.lockedCount?'<span class="locked-summary">'+iconSvg('lock')+data.lockedCount+' mã ngoài quyền xem</span>':'<span class="unlocked-summary">Đã mở toàn bộ nhóm</span>')+'</button>';
    }).join('');
    var planOptions=Object.keys(MOCK_PLAN_CONFIG).map(function(code){return '<option value="'+code+'" '+(state.mockPlan===code?'selected':'')+'>'+esc(MOCK_PLAN_CONFIG[code].label)+'</option>';}).join('');
    var capacityText=plan.fullMarket?watchlist.length+' mã ưu tiên':watchlist.length+'/'+plan.watchlistLimit+' mã';
    var quotaText=plan.changeLimit===null?'Không giới hạn quyền xem':remaining+'/'+plan.changeLimit+' lượt đổi còn lại';
    return '<main class="wrap overview-main overview-v19">'+
      '<section class="overview-section market-overview" aria-labelledby="market-overview-title"><div class="overview-section-head"><div><span class="eyebrow">MARKET INTELLIGENCE · MỌI GÓI</span><h1 id="market-overview-title">Toàn thị trường</h1><p>Aggregate thật từ toàn scanner universe hiện tại, không phải riêng Watchlist.</p></div><div class="universe-total"><small>Scanner universe</small><strong>'+total+' mã</strong></div></div><div class="market-metrics">'+marketMetrics+'<article class="market-metric universe-metric"><span>Tổng universe</span><strong>'+total+'</strong><small>Nguồn scanner hiện tại</small><b>Aggregate công khai</b></article></div></section>'+
      '<section class="overview-section watchlist-overview" aria-labelledby="watchlist-title"><div class="overview-section-head"><div><span class="eyebrow">PERSONAL STOCK INTELLIGENCE · STAGING MOCK</span><h2 id="watchlist-title">Watchlist của tôi</h2><p>Danh sách giả lập lấy động từ dữ liệu thật; không lưu Supabase và không thay scanner universe.</p></div><span class="mock-plan-badge">STAGING · '+esc(plan.label)+'</span></div><div class="watchlist-summary"><div><small>Đang theo dõi</small><strong>'+capacityText+'</strong></div><div><small>Đạt từ 3 tín hiệu</small><strong>'+watchlistStrong+' mã</strong></div><div><small>Lượt đổi</small><strong>'+quotaText+'</strong></div></div><div class="overview-card-grid">'+watchlist.slice(0,6).map(overviewStockCard).join('')+'</div>'+(watchlist.length>6?'<p class="preview-note">Đang xem nhanh 6/'+watchlist.length+' mã trong Watchlist staging.</p>':'')+'</section>'+
      '<section class="overview-section discovery-overview" aria-labelledby="discovery-title"><div class="overview-section-head"><div><span class="eyebrow">ENTITLEMENT-AWARE DISCOVERY</span><h2 id="discovery-title">Market Signal Discovery</h2><p>Count toàn thị trường luôn mở; ticker chi tiết chỉ hiện khi nằm trong quyền xem mock.</p></div></div><div class="discovery-grid">'+discoveryHtml+'</div></section>'+
      '<section class="overview-section entitlement-preview" aria-labelledby="entitlement-title"><div class="overview-section-head"><div><span class="eyebrow">STAGING ONLY</span><h2 id="entitlement-title">User Entitlement Preview</h2><p>Chuyển trạng thái mock để Product Owner kiểm tra coverage. Đây không phải tính năng tài khoản production.</p></div><label class="mock-plan-control"><span>Mock plan</span><select id="mock-plan-select" aria-label="Chọn gói giả lập staging">'+planOptions+'</select></label></div><div class="entitlement-strip"><div><small>Quyền xem chi tiết</small><strong>'+(plan.fullMarket?'Toàn thị trường':plan.viewLimit+' mã')+'</strong></div><div><small>Watchlist</small><strong>'+(plan.watchlistLimit===null?'Ưu tiên cá nhân':plan.watchlistLimit+' mã')+'</strong></div><div><small>Email</small><strong>'+(plan.email?'Có':'Không')+'</strong></div><div><small>Telegram</small><strong>'+(plan.telegram?'Có':'Không')+'</strong></div><div><small>Giá tham chiếu</small><strong>'+esc(plan.price)+'</strong></div></div>'+(plan.inherits?'<p class="inheritance-note">'+esc(plan.inherits)+'.</p>':'')+'</section>'+
      '<p class="disclaimer">STAGING UX preview · chưa có auth, payment hoặc secure entitlement backend. Công cụ quét dữ liệu, không đưa ra khuyến nghị mua/bán.</p></main>';
  }
