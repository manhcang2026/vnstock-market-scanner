(function () {
  ...
  function planModel() {
    ...
    var scannerSelected = state.route === "scanner" && state.scanner && state.scanner.watchlistLoadedFor
      ...
      : null;
    var selected = scannerSelected !== null ? scannerSelected : (overviewSelected || 0);
  ...
  function scannerPagerHtml(total) {
    ...
    var pages = Math.max(1, Math.ceil(total / scannerPageSize()));
    state.scanner.page = Math.min(state.scanner.page, pages);
    if (pages <= 1) return "";
    return '<div class="pager"><button type="button" data-scanner-page="-1"' + (state.scanner.page <= 1 ? " disabled" : "") + '>Trang trước</button><span>Trang ' + state.scanner.page + '/' + pages + '</span><button type="button" data-scanner-page="1"' + (state.scanner.page >= pages ? " disabled" : "") + '>Trang sau</button></div>';
  }
  ...
  function scannerWatchlistControlsHtml() {
    var buttons = [
      ["", "Tất cả", "all"],
      ["4of4", "4/4", "pass"],
      ["3plus", "≥3", "converge"],
    ...
    return '<div class="quick-filters scanner-signal-filters" aria-label="Bộ lọc tín hiệu DS của tôi">' + buttons + '</div>' +
  ...
  function scannerContextRailHtml(mode, filteredCount, shownCount) {
    if (mode === "market") {
      ...
      var body = '<dl class="rail-kv"><div><dt>Tổng mã thị trường</dt><dd>' + total + '</dd></div><div><dt>Khớp bộ lọc</dt><dd>' + filteredCount + '</dd></div><div><dt>Đang hiển thị</dt><dd>' + shownCount + '</dd></div></dl>' +
        '<p class="rail-note">' + iconSvg("shield") + '<span>Toàn bộ thị trường chỉ hiển thị dữ liệu cơ bản. Tín hiệu CCC không được tải ở chế độ này.</span></p>';
      return railCardHtml("Ngữ cảnh kết quả", "Toàn thị trường", body, "scanner-context-card") +
    ...
    var wBody = '<dl class="rail-kv"><div><dt>DS của tôi</dt><dd>' + state.scanner.watchlistRows.length + '</dd></div><div><dt>Khớp bộ lọc</dt><dd>' + filteredCount + '</dd></div><div><dt>Đang hiển thị</dt><dd>' + shownCount + '</dd></div></dl>' +
      '<p class="rail-note">' + iconSvg("shield") + '<span>Tín hiệu chỉ hiển thị cho các mã trong DS của bạn.</span></p>';
    return railCardHtml("Ngữ cảnh kết quả", "DS của tôi", wBody, "scanner-context-card") + planScopeCardHtml();
  }
  ...
  function scannerPageHtml() {
    ...
    var rows = mode === "market" ? scannerMarketFilteredRows() : scannerWatchlistFilteredRows();
    var visible = scannerVisibleRows(rows);
    var title = mode === "market" ? "Kết quả toàn thị trường" : "Kết quả DS của tôi";
    var resultBody = "";

    if (loading && !(mode === "market" ? state.scanner.marketLoaded : state.scanner.watchlistLoadedFor)) {
      ...
      resultBody = '<div class="scanner-table-wrap scanner-market-table-wrap"><table class="lovable-scanner-table scanner-market-table"><colgroup><col class="col-company"><col class="col-price"><col class="col-change"><col class="col-volume"><col class="col-ma200"><col class="col-ma10"></colgroup><thead><tr><th>Công ty</th><th class="num">Giá</th><th class="num">% thay đổi</th><th class="num">Khối lượng</th><th class="num">MA200</th><th class="num">MA10</th></tr></thead><tbody>' + visible.map(marketBasicRowHtml).join("") + '</tbody></table></div>' +
        '<div class="scanner-mobile-list scanner-market-mobile-list">' + visible.map(marketBasicCardHtml).join("") + '</div>' + scannerPagerHtml(rows.length);
    } else {
      resultBody = '<div class="scanner-table-wrap"><table class="lovable-scanner-table scanner-watchlist-table"><colgroup><col class="col-company"><col class="col-price"><col class="col-change"><col class="col-volume"><col class="col-ccc"></colgroup><thead><tr><th>Công ty</th><th class="num">Giá</th><th class="num">Thay đổi</th><th class="num">KL hiện tại</th><th>CCC</th></tr></thead><tbody>' + visible.map(watchlistTableRowHtml).join("") + '</tbody></table></div>' +
        '<div class="scanner-mobile-list">' + visible.map(stockRowHtml).join("") + '</div>' + scannerPagerHtml(rows.length);
    }

    var controls = mode === "market" ? scannerMarketControlsHtml() : scannerWatchlistControlsHtml();
    var rail = scannerContextRailHtml(mode, rows.length, visible.length);

    return '<main id="main-content" class="wrap scanner-main lovable-scanner page-shell">' +
      '<section class="scanner-controls panel-anatomy"><div class="scanner-mode" role="group" aria-label="Phạm vi dữ liệu"><button type="button" data-scanner-mode="market" class="' + (mode === "market" ? "active" : "") + '">Toàn bộ thị trường</button><button type="button" data-scanner-mode="watchlist" class="' + (mode === "watchlist" ? "active" : "") + '">DS của tôi</button></div>' +
      ...
      '<div class="scanner-mode-note">' + (mode === "market" ? '<strong>Toàn bộ thị trường:</strong> dữ liệu cơ bản của khoảng 800 mã cổ phiếu' : '<strong>DS của tôi:</strong> hiển thị đầy đủ tín hiệu cho các mã bạn đang theo dõi.') + '</div>' +
      controls + '</section>' +
      '<div class="scanner-layout"><div class="scanner-layout-main"><section class="scanner-results panel-anatomy"><header class="section-bar"><div><h2>' + title + '</h2><span>Tổng ' + rows.length + ' mã</span></div><small>' + visible.length + ' mã hiển thị</small></header>' + resultBody + '</section></div><aside class="context-rail scanner-context-rail">' + rail + '</aside></div>' +
      '<p class="disclaimer">STAGING · Scanner universe do backend quản lý; bộ lọc frontend không làm thay đổi danh sách quét.</p></main>';
  }
  ...
  async function ensureScanner(force) {
    ...
    try {
      if (state.scanner.mode === "market") {
        ...
        } else if (force || state.scanner.watchlistLoadedFor !== state.user.id) {
          var watchlistState = await window.CCCData.getMyOverviewState();
          state.scanner.watchlistRows = Array.isArray(watchlistState && watchlistState.rows) ? watchlistState.rows.slice() : [];
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-scanner-mode]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        state.scanner.sort = state.scanner.mode === "watchlist" ? "signal_desc" : "symbol";
        state.scanner.page = 1;
        state.scanner.mobileShown = MOBILE_CHUNK;
    ...
    document.querySelectorAll("[data-scanner-page]").forEach(function (button) {
      button.addEventListener("click", function () {
        state.scanner.page = Math.max(1, state.scanner.page + Number(button.getAttribute("data-scanner-page") || 0));
        render();
      });
    });
  ...
  window.addEventListener("resize", function () {
    if (state.route === "overview" || state.route === "scanner") render();
  });
