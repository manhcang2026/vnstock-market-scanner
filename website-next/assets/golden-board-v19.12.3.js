(function () {
  ...
  function renameFourSignalKpi() {
    ...
    root.querySelectorAll("span,strong,b,h2,h3,p").forEach(function (node) {
      ...
      if (text !== "4 tín hiệu" && text !== "Đạt 4/4") return;
      var holder = node.closest('[class*="kpi"],[class*="stat"],[class*="summary"],.density-tile');
  ...
  function guestRightRailHtml(data, teaserCount) {
    ...
    return '<aside class="gb-right-rail" aria-label="Thông tin Bảng vàng">' +
      '<section class="gb-rail-card">' +
        '<span class="eyebrow">CÁCH HIỂU</span>' +
        '<h3>Bảng vàng là gì?</h3>' +
        '<p>Một mã được ghi danh khi hệ thống đã ghi nhận đủ <b>4/4</b> trong phiên. Mã vẫn nằm lại trong lịch sử kể cả khi tín hiệu sau đó giảm.</p>' +
        '<div class="gb-rail-compare"><div><strong>Đang đạt 4/4</strong><span>Trạng thái ở lần quét gần nhất</span></div><div><strong>Bảng vàng</strong><span>Đã từng đạt 4/4 trong phiên</span></div></div>' +
      '</section>' +
      '<section class="gb-rail-card">' +
        '<span class="eyebrow">BẢN XEM THỬ</span>' +
        '<h3>Xem thêm Bảng vàng</h3>' +
        '<p>Bạn có thể xem tối đa <b>2 mã thật</b> sau ' + fmt(delay, 0) + ' phút. Chi tiết kỹ thuật đầy đủ vẫn theo quyền thành viên.</p>' +
        (hidden > 0 ? '<div class="gb-rail-number"><strong>' + fmt(hidden, 0) + '</strong><span>mã khác đang được giữ kín</span></div>' : '') +
        '<a class="gb-rail-action" href="' + ACCOUNT_PATH + '">Đăng nhập để xem thêm</a>' +
      '</section>' +
      '<section class="gb-rail-card gb-rail-note">' +
        '<span class="eyebrow">LƯU Ý</span>' +
        '<p>Bảng vàng là nhật ký tín hiệu, không phải khuyến nghị mua bán. Thời điểm hiển thị là lúc hệ thống ghi nhận, không khẳng định đó là điểm mua.</p>' +
      '</section>' +
    '</aside>';
  }

  function memberRightRailHtml(data, rows) {
    var week = Array.isArray(data.week_summary) ? data.week_summary : [];
    ...
    var still = (rows || []).filter(function (row) {
      return row.still_4of4 === true || Number(row.latest_signal_count || 0) === 4;
    }).length;
    var weekCard = '';
    if (leader && Number(leader.sessions_count || 0) >= 2) {
      weekCard = '<section class="gb-rail-card gb-rail-week"><span class="eyebrow">TUẦN NÀY</span><h3>🏆 Mã Vàng Tuần</h3>' +
        identityHtml(leader, true) +
        '<div class="gb-rail-week-grid"><span><b>' + fmt(leader.sessions_count, 0) + '</b> phiên</span><span><b>' + fmt(leader.best_streak_hits, 0) + '</b> chuỗi tốt nhất</span><span><b>' + fmt(leader.total_hits, 0) + '</b> lượt 4/4</span><span><b>' + pct(leader.avg_rvol30, 0).replace("+", "") + '</b> RVOL TB</span></div></section>';
    } else {
      weekCard = '<section class="gb-rail-card"><span class="eyebrow">TUẦN NÀY</span><h3>🏆 Mã Vàng Tuần</h3><p>Đang tích lũy thêm các phiên Bảng vàng để xếp hạng tuần.</p></section>';
    }
    return '<aside class="gb-right-rail" aria-label="Tóm tắt Bảng vàng">' +
      '<section class="gb-rail-card">' +
        '<span class="eyebrow">PHIÊN ĐANG XEM</span><h3>Tóm tắt</h3>' +
        '<div class="gb-rail-stats"><div><strong>' + fmt(marketTotal, 0) + '</strong><span>đã ghi danh</span></div><div><strong>' + fmt(still, 0) + '</strong><span>còn giữ 4/4</span></div><div><strong>' + fmt(accessible, 0) + '</strong><span>trong quyền xem</span></div>' + (hidden > 0 ? '<div><strong>' + fmt(hidden, 0) + '</strong><span>ngoài phạm vi</span></div>' : '') + '</div>' +
      '</section>' +
      weekCard +
      '<section class="gb-rail-card gb-rail-note"><span class="eyebrow">CÁCH ĐỌC</span><p><b>Đang đạt 4/4</b> là trạng thái hiện tại. <b>Bảng vàng</b> lưu mọi mã đã từng được ghi nhận 4/4 trong phiên.</p></section>' +
    '</aside>';
  }
  ...
  function fullPageHtml(payload) {
    ...
    if (!payload.signedIn) {
      ...
      return '<div class="page-shell golden-board-page">' +
        '<header class="page-header gb-page-header"><div class="page-heading"><div><span class="eyebrow">NHẬT KÝ TÍN HIỆU</span><h1>🏆 Bảng vàng</h1><p>Sổ ghi danh những mã đã được ghi nhận đạt đủ 4/4 trong phiên.</p></div><div class="gb-date-static">Phiên ' + esc(dateText(data.trading_date)) + "</div></div></header>" +
        '<div class="gb-content-grid">' +
          '<div class="gb-content-main">' +
            '<section class="gb-hero-grid gb-hero-grid-guest"><div class="gb-stat"><span>Đã ghi danh</span><strong>' + fmt(marketTotal, 0) + '</strong><small>mã trong phiên</small></div><div class="gb-stat"><span>Bản xem thử</span><strong>Tối đa 2 mã</strong><small>hé lộ sau ' + fmt(data.teaser_delay_minutes || 20, 0) + " phút</small></div></section>" +
            '<section class="gb-section"><div class="gb-section-head"><div><h2>Hé một chút Bảng vàng</h2><p>Một phần nhỏ để bạn thấy Bảng vàng đang ghi nhận gì. Dữ liệu kỹ thuật đầy đủ vẫn theo quyền thành viên.</p></div></div>' +
              (teasers.length ? '<div class="gb-teaser-list">' + teasers.map(teaserRowHtml).join("") + "</div>" : '<div class="gb-empty">Chưa có mã nào đủ ' + fmt(data.teaser_delay_minutes || 20, 0) + ' phút để hé lộ ở phiên này.</div>') +
            ...
            "</section>" +
          "</div>" +
          guestRightRailHtml(data, teasers.length) +
        "</div>" +
      "</div>";
    }

    return '<div class="page-shell golden-board-page">' +
      '<header class="page-header gb-page-header"><div class="page-heading"><div><span class="eyebrow">NHẬT KÝ TÍN HIỆU</span><h1>🏆 Bảng vàng</h1><p>Mã đã từng được hệ thống ghi nhận đạt đủ 4/4 sẽ được giữ lại để xem lịch sử, kể cả khi sau đó tín hiệu giảm.</p></div>' + datesHtml(data) + "</div></header>" +
      '<div class="gb-content-grid">' +
        '<div class="gb-content-main">' +
          '<section class="gb-hero-grid">' +
            ...
            '<div class="gb-stat"><span>Đang giữ 4/4</span><strong>' + fmt(still, 0) + '</strong><small>theo lần quét gần nhất</small></div>' +
          "</section>" +
  ...
  function overviewWidgetHtml(payload) {
    ...
    return '<section id="golden-board-overview" class="gb-overview-card">' +
      '<div class="gb-overview-head"><div><span class="eyebrow">TÍCH LŨY TRONG PHIÊN</span><h2>🏆 Bảng vàng hôm nay</h2><p>' + (marketTotal ? fmt(marketTotal, 0) + " mã đã được ghi nhận đạt đủ 4/4." : "Chưa có mã nào được ghi nhận đạt đủ 4/4 trong phiên.") + '</p></div><a href="' + PATH + '" data-golden-board-nav="1">Xem Bảng vàng →</a></div>' +
