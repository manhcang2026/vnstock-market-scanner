(function () {
  ...
  function ensureMobileRefreshIndicator() {
    var secondary = document.querySelector("#mobile-status-row .mobile-status-secondary");
    if (!secondary) return;

    var oldCountdown = document.getElementById("mobile-countdown");
    if (oldCountdown && oldCountdown.parentElement) oldCountdown.parentElement.style.display = "none";

    if (!document.getElementById("ccc-mobile-refresh-countdown")) {
      var wrap = document.createElement("span");
      wrap.className = "ccc-mobile-refresh-indicator";
      wrap.innerHTML = 'Làm mới <b id="ccc-mobile-refresh-countdown">05:00</b>';
      wrap.title = "Tự động kiểm tra dữ liệu mỗi 5 phút";
      secondary.appendChild(wrap);
    }
  }

  function patchUiPolish() {
    document.body.classList.add("ccc-ui-polish");
  ...
  async function loadMembership(renderAfter) {
    ...
    try {
      ...
      if (!state.previewPlanCode && state.plan) state.previewPlanCode = state.plan.plan_code;
  ...
  function planCatalogHtml() {
    ...
    var selectedCode = state.previewPlanCode || state.plan.plan_code;
    var selected = planCatalogItem(selectedCode) || state.plan;
    var currentCode = String(state.plan.plan_code || "");
    var choices = state.planCatalog.map(function (item) {
      var code = String(item.plan_code || "");
      var active = code === String(selected.plan_code || "");
      var current = code === currentCode;
      return '<button type="button" class="ccc-plan-choice ' + (active ? 'active ' : '') + (current ? 'current' : '') + '" data-plan-preview="' + esc(code) + '" aria-pressed="' + active + '">' +
        '<span>' + esc(item.display_name || code) + '</span>' +
        '<strong>' + esc(planPriceLabel(item)) + '</strong>' +
        (current ? '<small>Đang dùng</small>' : item.is_recommended ? '<small>Phổ biến</small>' : '<small>Xem gói</small>') +
      '</button>';
    }).join("");

    var isCurrent = String(selected.plan_code || "") === currentCode;
    var actionText = isCurrent ? "Đang sử dụng gói này" : "Đăng ký gói này";
    var actionNote = isCurrent ? "Gói đang áp dụng cho tài khoản của bạn." : "Thanh toán trực tuyến sẽ được nối ở bước billing sau.";

    return '<div class="ccc-plan-explorer">' +
      '<div class="ccc-plan-explorer-head"><div><span class="ccc-account-kicker">XEM & NÂNG CẤP GÓI</span><h3>Chọn gói để xem giá và quyền lợi</h3></div><span>Không thay đổi gói khi chỉ xem</span></div>' +
      '<div class="ccc-plan-choices" role="group" aria-label="Chọn gói để xem chi tiết">' + choices + '</div>' +
      '<div class="ccc-plan-preview">' +
        ...
        '</dl>' +
        '<button type="button" class="ccc-plan-register ' + (isCurrent ? 'current' : '') + '" disabled>' + esc(actionText) + '</button>' +
        '<p class="ccc-plan-register-note">' + esc(actionNote) + '</p>' +
      '</div>' +
    '</div>';
  }
