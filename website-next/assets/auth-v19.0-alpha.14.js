(function () {
  ...
  async function loadMa10CardData() {
    ...
    try {
      var response = await fetch(
        SUPABASE_URL + "/rest/v1/stock_snapshot?select=symbol,ma10_distance_pct&order=symbol.asc&limit=1000",
        {
          method: "GET",
          cache: "no-store",
          headers: {
      ...
      if (Array.isArray(rows)) {
        rows.forEach(function (row) {
          ...
          if (!symbol) return;
          var value = Number(row.ma10_distance_pct);
          ma10BySymbol[symbol] = Number.isFinite(value) ? value : null;
        });
  ...
  function patchMobileMa10Cards() {
    ...
    cards.forEach(function (card) {
      ...
      var value = Object.prototype.hasOwnProperty.call(ma10BySymbol, symbol) ? ma10BySymbol[symbol] : null;
      ...
      var tone = Number.isFinite(value) ? (value > 0 ? "positive" : value < 0 ? "negative" : "") : "";
      node.className = "ccc-card-ma10 " + tone;
      node.innerHTML = '<span>MA10</span><strong>' + esc(formatSignedPctOne(value)) + '</strong>';
    });
  }
  ...
  function patchMobileHeader433() {
    ...
    if (secondary.getAttribute("data-ccc433") !== "1") {
      var dataTimeNode = document.getElementById("mobile-data-time");
      var dataTime = dataTimeNode ? String(dataTimeNode.textContent || "—").trim() : "—";

      secondary.innerHTML =
        '<span class="ccc-mobile-data">Dữ liệu <b id="mobile-data-time">' + esc(dataTime) + '</b></span>' +
        '<span class="ccc-mobile-refresh-indicator" title="Tự động kiểm tra dữ liệu mỗi 5 phút">Làm mới <b id="ccc-mobile-refresh-countdown">05:00</b></span>';
      secondary.setAttribute("data-ccc433", "1");
    }
  }
