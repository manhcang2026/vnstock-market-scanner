(function () {
  ...
  function signalRailHtml(row) {
    ...
    return '<div class="signal-rail" aria-label="' + count + ' trên 4 tín hiệu"><div class="signal-segments">' + flags.map(function (value, index) { return '<span class="signal-segment ' + (value ? 'on' : '') + '" title="' + esc(labels[index]) + '"></span>'; }).join("") + '</div><strong>' + count + '/4</strong></div>';
  }
  ...
  function overviewRowHtml(row, sample) {
    ...
    return '<article class="overview-stock-row' + (sample ? ' is-sample' : '') + '">' +
      ...
      '<div class="overview-stock-ccc">' + signalRailHtml(row) + '<div class="overview-stock-evidence"><span>RVOL30 <b>' + (rvol === null ? '—' : formatNumber(rvol, 0) + '%') + '</b></span><span>MA200 <b class="' + tone(row.ma200_distance_pct) + '">' + pct(row.ma200_distance_pct, 1) + '</b></span></div></div>' +
    '</article>';
  }
  ...
  function overviewKpisHtml() {
    ...
    return '<section class="panel overview-density"><header class="overview-section-head"><div><span class="eyebrow">MẬT ĐỘ TÍN HIỆU HÔM NAY</span><h2>Sức nóng thị trường</h2></div><p>Số lớn là toàn thị trường. Dòng dưới cho biết DS của bạn.</p></header><div class="overview-kpi-grid">' + keys.map(function (key) {
      ...
      return '<button type="button" class="overview-kpi-card' + (active ? ' active' : '') + '" data-overview-group="' + key + '" aria-pressed="' + (active ? 'true' : 'false') + '"><span>' + esc(cfg.label) + '</span><strong>' + market + '<small> mã</small></strong><footer><em>Toàn thị trường</em><b>' + esc(mineText) + '</b></footer></button>';
    }).join("") + '</div></section>';
  }
  ...
  function overviewResultsHtml() {
    ...
    return '<section class="panel overview-results"><header class="overview-results-head"><div><span class="eyebrow">' + (sample ? 'TRẢI NGHIỆM' : 'DS CỦA BẠN') + '</span><h2>' + esc(title) + '</h2><p>' + esc(meta) + '</p></div>' + reset + '</header>' + intro + '<div class="overview-row-head"><span>Công ty</span><span>Giá / thay đổi</span><span>Khối lượng</span><span>CCC</span></div><div class="overview-rows">' + body + '</div></section>' + overviewUpsellHtml();
  }
  ...
  function marketPulseRailHtml() {
    ...
    return '<section class="rail-card market-pulse-card"><header><span>TOÀN CẢNH THỊ TRƯỜNG</span><b>' + (rows.some(function (r) { return r.market_status === 'OPEN' || r.market_status === 'OPEN_24H'; }) ? 'LIVE' : 'CẬP NHẬT') + '</b></header><div class="market-pulse-list">' + rows.map(function (row) { return '<div><span>' + esc(row.display_name || row.key) + '</span><strong>' + formatNumber(row.price, Number(row.price) < 100 ? 2 : 0) + '</strong><b class="' + tone(row.change_pct) + '">' + pct(row.change_pct, 2) + '</b></div>'; }).join("") + '</div></section>';
  }
  ...
  function mountShell() {
    app.innerHTML = '' +
      '<header class="app-header">' +
        ...
        '</div>' +
      '</header>' +
      '<aside class="desktop-sidebar" aria-label="Thanh điều hướng"><nav id="desktop-nav-links" aria-label="Điều hướng chính"></nav><div class="sidebar-footer"><div id="desktop-account-link"></div><span class="staging-badge"><strong>STAGING</strong><small>v19.1.1-overview</small></span></div></aside>' +
  ...
  function overviewPage() {
    ...
    var rail = marketPulseRailHtml() + overviewTrustRailHtml();
