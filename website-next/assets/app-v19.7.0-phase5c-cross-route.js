(function () {
  ...
  var STOCK_LOGO_LOCAL_BASE = "/assets/logos";
  var STOCK_LOGO_REMOTE_FALLBACK = "https://www.chuyenchochung.com/stock-logos";
  ...
  var state = {
    ...
    scanner: {
      mode: scannerModeFromLocation() || "market",
      loading: false,
      error: "",
      marketLoaded: false,
      marketRows: [],
      ...
      exchange: "all",
      sort: new URLSearchParams(location.search).get("sort") || ((scannerModeFromLocation() || "market") === "watchlist" ? "signal_desc" : "symbol"),
  ...
  var initialDetailLocation = detailLocationState();
  if (initialDetailLocation) {
    var existingDetailHistory = history.state && typeof history.state === "object"
      ? history.state
      : {};
    var restoredFromApp = existingDetailHistory.cccDetail === true &&
      existingDetailHistory.direct !== true &&
      typeof existingDetailHistory.sourcePath === "string";
    var restoredSourcePath = restoredFromApp ? existingDetailHistory.sourcePath : "/";
    var restoredSourceRoute = restoredFromApp
      ? (existingDetailHistory.sourceRoute || "overview")
      : "overview";
    var restoredScroll = restoredFromApp ? Number(existingDetailHistory.returnScroll || 0) : 0;

    state.route = restoredSourceRoute;
    state.detail.open = true;
    state.detail.symbol = initialDetailLocation.symbol;
    state.detail.tab = initialDetailLocation.tab;
    state.detail.returnPath = restoredSourcePath;
    state.detail.sourceRoute = restoredSourceRoute;
    state.detail.returnScroll = restoredScroll;
    state.detail.openedFromApp = restoredFromApp;

    history.replaceState({
      cccDetail: true,
      symbol: initialDetailLocation.symbol,
      tab: initialDetailLocation.tab,
      sourcePath: restoredSourcePath,
      sourceRoute: restoredSourceRoute,
      returnScroll: restoredScroll,
      direct: !restoredFromApp
    }, "", detailUrl(initialDetailLocation.symbol, initialDetailLocation.tab));
  }

  function normalizeDetailTab(value) {
    var tab = String(value || "").toLowerCase();
    return ["overview","technical","fundamental","financials"].indexOf(tab) >= 0 ? tab : "overview";
  }

  function detailLocationState() {
    var path = location.pathname || "/";
    var match = path.match(/^\/co-phieu\/([A-Za-z0-9]{1,12})\/?$/);
    if (!match) return null;
    return {
      symbol: String(match[1] || "").toUpperCase(),
      tab: normalizeDetailTab(new URLSearchParams(location.search).get("tab"))
    };
  }

  function detailUrl(symbol, tab) {
    var safe = String(symbol || "").trim().toUpperCase().replace(/[^A-Z0-9]/g, "");
    if (!safe) return "/";
    var url = "/co-phieu/" + encodeURIComponent(safe);
    var normalizedTab = normalizeDetailTab(tab);
    if (normalizedTab !== "overview") url += "?tab=" + encodeURIComponent(normalizedTab);
    return url;
  }
  ...
  function scannerModeFromLocation() {
    var raw = String(new URLSearchParams(location.search).get("mode") || "").toLowerCase();
    return raw === "watchlist" || raw === "market" ? raw : "";
  }

  function scannerPreferredMode() {
    return state.user ? "watchlist" : "market";
  }

  function scannerDefaultSortForMode(mode) {
    return mode === "watchlist" || (mode === "market" && effectiveFullMarketAccess())
      ? "signal_desc"
      : "symbol";
  }

  function applyScannerModeFromSession() {
    if (state.route !== "scanner") return;
    var explicitMode = scannerModeFromLocation();
    var nextMode = explicitMode || scannerPreferredMode();
    var changed = state.scanner.mode !== nextMode;
    state.scanner.mode = nextMode;

    var explicitSort = String(new URLSearchParams(location.search).get("sort") || "");
    if (!explicitSort || changed) {
      state.scanner.sort = explicitSort || scannerDefaultSortForMode(nextMode);
    }
  ...
  function writeScannerModeToUrl() {
    if (state.route !== "scanner") return;
    var params = new URLSearchParams(location.search);
    params.set("mode", state.scanner.mode);
    params.delete("signal");
    params.set("sort", state.scanner.sort);
    var next = (location.pathname || "/danh-sach") + (params.toString() ? "?" + params.toString() : "");
    history.replaceState(history.state || null, "", next);
  }

  function stockLogoBaseUrl() {
    var host = String(location.hostname || "").toLowerCase();
    if (host === "localhost" || host === "127.0.0.1" || host === "0.0.0.0") {
      return STOCK_LOGO_LOCAL_BASE;
    }
    var meta = document.querySelector('meta[name="ccc-stock-logo-base"]');
    var configured = meta && String(meta.getAttribute("content") || "").trim();
    return (configured || STOCK_LOGO_REMOTE_FALLBACK).replace(/\/+$/, "");
  }

  function stockLogoUrl(symbol) {
    var safe = String(symbol || "").toUpperCase().replace(/[^A-Z0-9]/g, "");
    return stockLogoBaseUrl() + "/" + encodeURIComponent(safe) + ".webp?v=1991-phase6bcd-final";
  }
  ...
  function companyLogo(symbol) {
    ...
    return '<span class="company-logo row-logo"><img class="company-logo-img" loading="lazy" decoding="async" fetchpriority="low" src="' + esc(stockLogoUrl(safe)) + '" alt="" onerror="this.style.display=\'none\'"/><span class="company-logo-fallback">' + esc(label) + '</span></span>';
  }
  ...
  function overviewSeeAllHtml(shownCount) {
    ...
    if (state.overview.group === "all") {
      label = full
        ? "Mở toàn bộ thị trường · " + total + " mã"
  ...
  function marketPulseRailHtml() {
    ...
    var cards = instruments.map(function (item) {
      ...
      var body = hasData ?
        ...
        '<div class="market-unavailable"><b>—</b><span>Đang chờ dữ liệu hệ thống</span></div>';
  ...
  function overviewBodyHtml() {
    ...
    var densityCopy = sampleMode()
      ...
      : (memberEmptyWatchlistMode()
        ...
        : '<span>Tổng quan chỉ hiển thị tối đa 10 mã nổi bật; vào Danh sách mã để xem và lọc toàn bộ.</span>');
    var densityMeta = sampleMode() ? "Bản trải nghiệm" : (memberEmptyWatchlistMode() ? "Chưa có DS" : "10 mã nổi bật theo bộ lọc");
  ...
  function scannerLatestRow(rows) {
    var latest = null;
    ...
    return latest || ((Array.isArray(rows) && rows.length) ? rows[0] : null);
  }

  function scannerTimeSlotLabel(value) {
    var text = String(value || "").trim();
    var match = text.match(/(\d{2}:\d{2})/);
    return match ? match[1] : (text || "—");
  }

  function scannerDataStatusLabel(rows) {
    var list = Array.isArray(rows) ? rows : [];
    if (!list.length) return "—";
    var ok = list.filter(function (row) {
      return String(row && row.data_status || "").toUpperCase() === "OK";
    }).length;
    return ok === list.length ? "Ổn định" : ok + "/" + list.length + " mã ổn định";
  }

  function scannerSourceCardHtml(rows, scopeTotal, technical) {
    var list = Array.isArray(rows) ? rows : [];
    var latest = scannerLatestRow(list) || {};
    var updated = latest.updated_at ? formatDateTime(latest.updated_at) : "—";
    var tradingDate = latest.trading_date ? formatDate(latest.trading_date) : "—";
    var slot = scannerTimeSlotLabel(latest.time_slot);
    var coverage = Number(scopeTotal || 0);

    var body = '<dl class="rail-kv">' +
      '<div><dt>Cập nhật gần nhất</dt><dd>' + esc(updated) + '</dd></div>' +
      '<div><dt>Ngày giao dịch</dt><dd>' + esc(tradingDate) + '</dd></div>' +
      '<div><dt>Mốc dữ liệu</dt><dd>' + esc(slot) + '</dd></div>' +
      '<div><dt>Độ phủ</dt><dd>' + (coverage ? coverage + ' mã' : '—') + '</dd></div>' +
      '<div><dt>Trạng thái</dt><dd>' + esc(scannerDataStatusLabel(list)) + '</dd></div>' +
      '<div><dt>Nguồn</dt><dd>Chuyện Chợ Chứng</dd></div>' +
    '</dl>' +
    '<p class="rail-note">' + iconSvg("shield") + '<span>' +
      (technical
        ? 'Dữ liệu kỹ thuật gồm KLTB10, MA10, MA200, RVOL30 và 4 tín hiệu CCC. MA10 chỉ là tham chiếu, không phải tín hiệu.'
        : 'Dữ liệu cơ bản công khai gồm giá, % thay đổi, khối lượng, MA10 và MA200; không hiển thị tín hiệu CCC hay RVOL30.') +
    '</span></p>';

    return railCardHtml("Nguồn dữ liệu", technical ? "Kỹ thuật CCC" : "Dữ liệu cơ bản", body, "scanner-source-card scanner-source-card-detailed");
  }

  function scannerBasicScopeCardHtml(total, filteredCount, shownCount) {
    var filtered = Number(filteredCount || 0);
    var showing = Number(shownCount || 0);
    var body = '<dl class="rail-kv">' +
      '<div><dt>Phạm vi</dt><dd>' + Number(total || 0) + ' mã</dd></div>' +
      '<div><dt>Sàn giao dịch</dt><dd>HOSE · HNX · UPCoM</dd></div>' +
      '<div><dt>Chế độ</dt><dd>Dữ liệu cơ bản</dd></div>' +
      (filtered !== Number(total || 0) ? '<div><dt>Sau bộ lọc</dt><dd>' + filtered + ' mã</dd></div>' : '') +
      '<div><dt>Đang hiển thị</dt><dd>' + showing + ' mã</dd></div>' +
    '</dl>' +
    '<p class="rail-note">' + iconSvg("shield") + '<span>Phù hợp để tra cứu nhanh toàn thị trường. Tín hiệu kỹ thuật CCC chỉ xuất hiện khi quyền xem của bạn cho phép.</span></p>';
    return railCardHtml("Phạm vi thị trường", "Toàn bộ thị trường", body, "scanner-market-scope-card");
  }

  function scannerMarketActionCardHtml() {
    var meta = state.user ? planProductName(basePlanCode()) : "Khách";
    var body;

    if (!state.user) {
      body = '<div class="scanner-action-copy"><strong>Tạo DS mã theo dõi của riêng bạn</strong><p>Tạo tài khoản FREE để chọn 10 mã quan tâm và xem tín hiệu CCC trong phạm vi cá nhân.</p></div>' +
        '<div class="scanner-rail-actions"><button type="button" class="rail-action scanner-rail-primary" data-guest-auth>Tạo tài khoản / Đăng nhập</button>' +
        '<button type="button" class="scanner-rail-secondary" data-upsell-vip>Xem VIP DAY · 24h</button></div>';
    } else {
      body = '<div class="scanner-action-copy"><strong>Mở kỹ thuật CCC toàn thị trường</strong><p>FULL mở kỹ thuật toàn thị trường. VIP DAY mở quyền tương đương FULL trong 24 giờ mà không thay đổi DS cá nhân.</p></div>' +
        '<div class="scanner-rail-actions"><button type="button" class="rail-action scanner-rail-primary" data-upsell-upgrade>Xem các gói</button>' +
        '<button type="button" class="scanner-rail-secondary" data-upsell-vip>Mở VIP DAY · 24h</button></div>';
    }

    return railCardHtml("Mở rộng quyền xem", meta, body, "scanner-action-card");
  }
  ...
  function scannerContextRailHtml(mode, filteredCount, shownCount) {
    if (scannerTechnicalMode()) {
      ...
      var accessLabel = isMarket
        ? (vipDayActive() ? "VIP DAY · Toàn thị trường" : "FULL · Toàn thị trường")
        : "DS của tôi";

      var body = '<dl class="rail-kv">' +
        '<div><dt>' + (isMarket ? "Phạm vi thị trường" : "DS của tôi") + '</dt><dd>' + scopeTotal + ' mã</dd></div>' +
        '<div><dt>Khớp bộ lọc</dt><dd>' + matching + ' mã</dd></div>' +
        '<div><dt>Trang hiện tại</dt><dd>' + shownCount + ' mã</dd></div>' +
        (isMarket ? '<div><dt>DS cá nhân</dt><dd>' + personalCount + ' mã</dd></div>' : '') +
        '</dl>' +
        '<p class="rail-note">' + iconSvg("shield") + '<span>' +
          (isMarket
            ? 'Bạn đang xem kỹ thuật CCC trên toàn thị trường; DS cá nhân vẫn giữ nguyên.'
            : 'Đây là đúng các mã trong DS cá nhân của bạn; bộ lọc chỉ sàng lọc bên trong phạm vi này.') +
        '</span></p>';

      return railCardHtml(isMarket ? "Phạm vi kỹ thuật" : "Phạm vi của tôi", accessLabel, body, "scanner-context-card") +
        planScopeCardHtml() +
        scannerSourceCardHtml(state.scanner.technicalRows, scopeTotal, true);
    }

    if (mode === "market") {
      var total = state.scanner.marketRows.length;
      return scannerBasicScopeCardHtml(total, filteredCount, shownCount) +
        scannerMarketActionCardHtml() +
        scannerSourceCardHtml(state.scanner.marketRows, total, false);
    }
  ...
  function phase6StableRailSkeletonHtml(extraClass) {
    var cls = extraClass ? " " + extraClass : "";
    return '<div class="phase6-stable-rail-skeleton' + cls + '" aria-hidden="true">' +
      '<section class="rail-card phase6-stable-skeleton-card"><span></span><i></i><i></i><i></i></section>' +
      '<section class="rail-card phase6-stable-skeleton-card"><span></span><i></i><i></i></section>' +
      '<section class="rail-card phase6-stable-skeleton-card"><span></span><i></i><i></i><i></i></section>' +
    '</div>';
  }

  function scannerSessionPendingHtml() {
    return '<main id="main-content" class="wrap scanner-main lovable-scanner page-shell phase6-stable-loading-shell" aria-busy="true">' +
      '<section class="scanner-controls panel-anatomy phase6-scanner-session-pending">' +
        '<div class="scanner-mode phase6-pending-tabs" aria-hidden="true"><span></span><span></span></div>' +
        '<div class="phase6-pending-search" aria-hidden="true"><span></span><i></i><i></i></div>' +
        '<div class="scanner-mode-note"><strong>Đang xác định phạm vi tài khoản…</strong> Danh sách sẽ mở đúng chế độ ngay khi phiên đăng nhập sẵn sàng.</div>' +
        '<div class="phase6-pending-filter-row" aria-hidden="true"><span></span><span></span><span></span></div>' +
      '</section>' +
      '<div class="scanner-layout"><div class="scanner-layout-main"><section class="scanner-results panel-anatomy">' +
  ...
  function scannerPageHtml() {
    if (!state.authReady) return scannerSessionPendingHtml();
    ...
    if (technical) {
      ...
      visible = rows;
      title = mode === "market" ? "Toàn bộ thị trường" : "Kết quả DS của tôi";
    } else {
    ...
    if (mode === "market" && technical) {
      ...
      modeNote = '<strong>Toàn bộ thị trường:</strong> dữ liệu cơ bản của khoảng 800 mã. FULL hoặc VIP DAY mở kỹ thuật CCC toàn thị trường.';
    } else {
    ...
    return '<main id="main-content" class="wrap scanner-main lovable-scanner page-shell">' +
      ...
      (state.user
        ? '<button type="button" data-scanner-mode="watchlist" class="' + (mode === "watchlist" ? "active" : "") + '">DS của tôi<small class="scanner-mode-count">' + scannerPersonalWatchlistCount() + '</small></button>' +
        ...
        : '<button type="button" data-scanner-mode="market" class="' + (mode === "market" ? "active" : "") + '">Toàn bộ thị trường</button>' +
          '<button type="button" data-scanner-mode="watchlist" class="' + (mode === "watchlist" ? "active" : "") + '">DS của tôi</button>') + '</div>' +
  ...
  function detailReset(keepScroll) {
    ...
    state.detail.returnScroll = 0;
    state.detail.returnPath = "/";
    state.detail.sourceRoute = "overview";
    state.detail.openedFromApp = false;
    if (keepScroll) {
      requestAnimationFrame(function () { window.scrollTo(0, previousScroll); });
    }
  }

  function applyDetailLocation(symbol, options) {
    options = options || {};
    ...
    state.detail.tab = normalizeDetailTab(options.tab);
    state.detail.loading = false;
    state.detail.error = "";
    state.detail.basic = null;
    state.detail.metadata = null;
    ...
    state.detail.watchlistBusy = false;
    state.detail.watchlistError = "";
    state.detail.returnScroll = Number(options.returnScroll || 0);
    state.detail.returnPath = options.returnPath || "/";
    state.detail.sourceRoute = options.sourceRoute || "overview";
    state.detail.openedFromApp = options.openedFromApp === true;

    if (options.canonicalize !== false && currentPathAndQuery() !== detailUrl(symbol, state.detail.tab)) {
      history.replaceState(history.state || null, "", detailUrl(symbol, state.detail.tab));
    }
  }

  function openStockDetail(symbol, options) {
    options = options || {};
    symbol = String(symbol || "").trim().toUpperCase();
    if (!symbol) return;

    if (options.fromLocation === true) {
      applyDetailLocation(symbol, {
        tab: options.tab,
        returnScroll: options.returnScroll,
        returnPath: options.returnPath,
        sourceRoute: options.sourceRoute,
        openedFromApp: options.openedFromApp,
        canonicalize: options.canonicalize
      });
      render();
      if (state.authReady) ensureDetail(false);
      return;
    }

    var sourcePath = currentPathAndQuery();
    var sourceRoute = state.route;
    var sourceScroll = window.scrollY || 0;
    var nextUrl = detailUrl(symbol, options.tab || "overview");

    history.pushState({
      cccDetail: true,
      symbol: symbol,
      tab: normalizeDetailTab(options.tab),
      sourcePath: sourcePath,
      sourceRoute: sourceRoute,
      returnScroll: sourceScroll,
      direct: false
    }, "", nextUrl);
  ...
  function closeStockDetail() {
    ...
    if (state.detail.openedFromApp) {
      history.back();
      return;
    }

    var fallback = state.detail.returnPath && state.detail.returnPath.indexOf("/co-phieu/") !== 0
      ? state.detail.returnPath
      : "/";
    var previousScroll = state.detail.returnScroll || 0;
  ...
  function updateDetailTab(tab) {
    if (!state.detail.open) return;
    state.detail.tab = normalizeDetailTab(tab);

    var currentState = history.state && typeof history.state === "object"
      ? Object.assign({}, history.state)
      : {};
    currentState.cccDetail = true;
    currentState.symbol = state.detail.symbol;
    currentState.tab = state.detail.tab;
    currentState.sourcePath = state.detail.returnPath;
    currentState.sourceRoute = state.detail.sourceRoute;
    currentState.returnScroll = state.detail.returnScroll;
    currentState.direct = !state.detail.openedFromApp;

    history.replaceState(currentState, "", detailUrl(state.detail.symbol, state.detail.tab));
    render();
  ...
  function syncFromBrowserHistory(eventState) {
    var detailLoc = detailLocationState();

    if (detailLoc) {
      var h = eventState && typeof eventState === "object" ? eventState : {};
      state.route = h.sourceRoute || state.route || "overview";
      openStockDetail(detailLoc.symbol, {
        fromLocation: true,
        tab: detailLoc.tab,
        returnScroll: Number(h.returnScroll || 0),
        returnPath: h.sourcePath || "/",
        sourceRoute: h.sourceRoute || state.route || "overview",
        openedFromApp: h.direct !== true && !!h.sourcePath,
        canonicalize: true
      });
      return;
    }

    var restoreScroll = state.detail.open ? state.detail.returnScroll || 0 : 0;
  ...
  function detailAccessLabel() {
    if (state.detail.technicalAllowed) {
      if (state.detail.technicalReason === "GUEST_DEMO") return "Mã trải nghiệm · Được mở";
    ...
    return state.user ? "Ngoài DS của tôi" : "Ngoài 10 mã trải nghiệm";
  }
  ...
  function detailOverviewHtml() {
    ...
    return '<section class="detail-panel"><header><h3>Báo giá công khai</h3><span>Luôn hiển thị cho mọi mã</span></header><div class="detail-metric-grid">' +
      ...
      '</div></section>' +
      '<section class="detail-panel"><header><h3>Bối cảnh cơ bản</h3><span>Nghiên cứu cơ bản công khai</span></header>' +
      ...
      '</section><div class="detail-notice">' + iconSvg("lock") + '<span>MA200, KLTB10, RVOL30 và thanh tín hiệu CCC nằm trong tab <b>Kỹ thuật</b>.</span></div>';
  }
  ...
  function detailTechnicalHtml() {
    if (!state.detail.technicalAllowed) {
      ...
      var copy = state.user
        ...
        : state.detail.symbol + " không thuộc 10 mã trải nghiệm của bản dùng thử. Cơ bản và BCTC vẫn được xem bình thường; đăng nhập để tạo DS riêng.";
    ...
    return '<section class="detail-panel"><header><h3>Thanh tín hiệu CCC</h3><span>Trạng thái dữ liệu hiện tại</span></header><div class="detail-rail-hero">' +
  ...
  function detailFundamentalHtml() {
    var f = state.detail.financial;
    if (!f) {
      return '<div class="empty-state"><strong>Chưa có dữ liệu cơ bản</strong><span>Dữ liệu cơ bản cho mã này chưa có trong hệ thống.</span></div>';
    }
  ...
  function detailContextRailHtml() {
    ...
    var sourceBody = '<dl class="rail-kv"><div><dt>Cập nhật</dt><dd>' + esc(formatDateTime(basic.updated_at)) + '</dd></div><div><dt>Trạng thái</dt><dd>' +
      esc(basic.data_status ? friendlyDataStatus(basic.data_status) : "—") + '</dd></div><div><dt>Nguồn</dt><dd>Chuyện Chợ Chứng</dd></div></dl>';
  ...
  function stockDetailPageHtml() {
    ...
    if ((state.detail.loading || !state.authReady || (!state.detail.basic && !state.detail.error)) && !state.detail.basic) {
  ...
  function accountLoggedOutHtml() {
    return '<main id="main-content" class="wrap ccc-account-page phase4-account-page">' +
      '<header class="ccc-account-heading phase5c-route-heading"><div><h1>Tài khoản</h1><p>Đăng nhập để quản lý hồ sơ, gói thành viên và thiết lập cá nhân.</p></div></header>' +
  ...
  function accountWatchlistHtml() {
    ...
    if (state.account.loading && !state.account.watchlist) {
      return '<section class="ccc-account-card ccc-watchlist-main-card phase4-watchlist-card"><header class="ccc-account-card-head"><div><span class="ccc-account-kicker">DS MÃ THEO DÕI & LƯỢT ĐỔI</span><h2>Danh sách mã theo dõi</h2><p>Đang đọc danh sách và hạn mức thật từ hệ thống.</p></div><span class="ccc-account-status pending">Đang tải</span></header><div class="ccc-wl-loading"><span class="transplant-spinner"></span><span>Đang tải…</span></div></section>';
    }
  ...
  function accountUpgradeHtml() {
    ...
    '</div><div class="phase4-upgrade-note">Chức năng chọn gói và thanh toán sẽ được nối ở bước thanh toán sau.</div></div>';
  }
  ...
  function accountVipDayHtml() {
    ...
    return '<section id="vip-day" class="ccc-vip-day-offer phase4-vip' + (new URLSearchParams(location.search).get("intent") === "vip_day" ? " is-intent" : "") + '">' +
      ...
      (state.account.vipInfoOpen
        ? '<p class="phase4-vip-note">Thanh toán thật chưa được mở ở bước này. Hệ thống VIP DAY đã sẵn sàng; sau khi nối thanh toán, nút này sẽ kích hoạt quyền khi giao dịch thành công.</p>'
        : '') +
    '</section>';
  }
  ...
  function accountPageHtml() {
    ...
    return '<main id="main-content" class="wrap ccc-account-page phase4-account-page">' +
      '<header class="ccc-account-heading phase5c-route-heading"><div><h1>Tài khoản</h1><p>Quản lý hồ sơ, gói thành viên và thiết lập cá nhân.</p></div><div class="ccc-account-identity"><span class="ccc-account-avatar">' + esc(userInitial()) + '</span><div><strong>' + esc(displayUserName()) + '</strong><span>' + esc(state.user.email || "") + '</span></div></div></header>' +
      '<div class="phase4-account-layout"><div class="phase4-account-main">' + accountProfileHtml() + accountWatchlistHtml() + '</div><aside class="phase4-account-rail">' + accountPlanHtml() + accountAlertChannelsHtml() + accountSecurityHtml() + '</aside></div>' +
    '</main>';
  }
  ...
  /* ==========================================================
     Phase 5B — Nghiên cứu cơ bản công khai
     Alpha.19 is the visual donor; Phase 4/5A remains the clean runtime.
     No CCC kỹ thuật CCC is mixed into these surfaces.
     ========================================================== */

  function researchMode() {
    return normalizedAppPath() === "/sang-loc-co-ban" ? "fundamental" : "industry";
  }
  ...
  function friendlyDataStatus(value) {
    var raw = String(value || "").trim().toUpperCase();
    if (!raw) return "Chưa rõ";
    if (["OK", "FRESH", "CURRENT"].indexOf(raw) >= 0) return "Ổn định";
    if (["LAGGING", "DELAYED"].indexOf(raw) >= 0) return "Chậm cập nhật";
    if (["STALE", "OLD"].indexOf(raw) >= 0) return "Cần cập nhật";
    if (["NO_DATA", "NO_FINANCIAL_DATA", "MISSING"].indexOf(raw) >= 0) return "Thiếu dữ liệu";
    if (raw === "PARTIAL") return "Chưa đầy đủ";
    if (raw === "ERROR") return "Lỗi dữ liệu";
    return "Chưa rõ";
  }
  ...
  function researchHeroHtml(title, copy) {
    return '<section class="page-heading research-hero page-header phase5c-route-heading"><div><h1>' + esc(title) + '</h1><p>' + esc(copy) + '</p></div></section>';
  }
  ...
  function researchLoadingHtml() {
    return '<main id="main-content" class="wrap fund-main research-main page-shell has-context-rail phase6-stable-loading-shell" aria-busy="true">' +
      researchHeroHtml(researchMode() === 'industry' ? 'So sánh theo ngành' : 'Sàng lọc cơ bản', 'Đang tải dữ liệu cơ bản công khai…') +
      researchTabsHtml() +
      '<div class="content-grid phase5b-research-grid"><div class="content-main"><section class="panel-anatomy transplant-state phase6-research-loading-panel"><span class="transplant-spinner"></span><div><strong>Đang tải Nghiên cứu…</strong><p>Đang lấy dữ liệu tài chính và tên doanh nghiệp.</p></div></section></div><aside class="context-rail research-context-rail">' +
      phase6StableRailSkeletonHtml('phase6-research-rail-pending') +
      '</aside></div>' +
      '<p class="disclaimer">Điểm số hỗ trợ sàng lọc và học phân tích, không phải khuyến nghị mua/bán.</p></main>';
  }
  ...
  function researchErrorHtml() {
    ...
    return '<main id="main-content" class="wrap fund-main research-main page-shell has-context-rail phase6-stable-loading-shell">' +
      researchHeroHtml(title, copy) + researchTabsHtml() +
      '<div class="content-grid phase5b-research-grid"><div class="content-main"><section class="panel-anatomy transplant-state is-error"><div><strong>Không tải được dữ liệu Nghiên cứu.</strong><p>' + esc(state.research.error || 'Nguồn dữ liệu tạm thời chưa phản hồi.') + '</p><button type="button" class="secondary-action" data-research-retry>Thử lại</button></div></section></div><aside class="context-rail research-context-rail">' +
      phase6StableRailSkeletonHtml('phase6-research-rail-pending') +
      '</aside></div>' +
      '<p class="disclaimer">Điểm số hỗ trợ sàng lọc và học phân tích, không phải khuyến nghị mua/bán.</p></main>';
  }
  ...
  async function initAuth() {
    try {
      ...
      render();
      if (state.detail.open) ensureDetail(false);
      else {
        if (state.route === "overview") ensureOverview(false);
        if (state.route === "scanner") ensureScanner(false);
        if (state.route === "research") ensureResearch(false);
  ...
  async function submitSearch(form) {
    ...
    var rows = await ensureSearchUniverse();
    var ranked = rankStockSearch(rows, query, 1);
    if (ranked.length && ranked[0].symbol) {
      openStockDetail(ranked[0].symbol);
      return;
    }

    location.assign("/danh-sach?q=" + encodeURIComponent(query));
  }
  ...
  function bind() {
    ...
    document.querySelectorAll("[data-scanner-mode]").forEach(function (button) {
      button.addEventListener("click", function () {
        ...
        state.scanner.signal = "";
        state.scanner.sort = scannerDefaultSortForMode(state.scanner.mode);
        state.scanner.page = 1;
        state.scanner.mobileShown = MOBILE_CHUNK;
        state.scanner.mobileDropdown = "";
        state.scanner.technicalLoadedKey = "";
        writeScannerModeToUrl();
        render();
        ensureScanner(true);
      });
    });
    ...
    document.querySelectorAll("[data-symbol]").forEach(function (element) {
      ...
      if (element.tagName === "TR") {
        element.setAttribute("aria-label", "Mở chi tiết " + symbol);
      } else {
      ...
      function activateDetail(event) {
        ...
        if (event && event.type === "click" && event.target && typeof event.target.closest === "function") {
          var interactive = event.target.closest("a,button,input,select,textarea,[role=button]");
    ...
    document.querySelectorAll("[data-detail-tab]").forEach(function (button) {
      button.addEventListener("click", function () {
        updateDetailTab(button.getAttribute("data-detail-tab") || "overview");
      });
    });
  ...
  window.addEventListener("popstate", function (event) {
    syncFromBrowserHistory(event && event.state || null);
  });
