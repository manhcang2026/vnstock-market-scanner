(function (global) {
  ...
  async function loadResearch(force) {
    ...
    /*
      Public Fundamental Research must not inherit the signed-in user's
      technical/member RLS role. financial_latest is intentionally public
      through the anon policy, so read Research through the same public REST
      path already used by Stock Detail fundamentals.
    */
    researchPromise = Promise.all([
      publicRestRows("financial_latest", {
      ...
      return {
        financial: Array.isArray(results[0]) ? results[0] : [],
        metadata: Array.isArray(results[1]) ? results[1] : []
      };
    }).catch(function (error) {
